...
export function useVoiceAssistant() {
    ...
    const sendMessage = useCallback(async (message: string, audible: boolean) => {
        ...
        const runCompletionsLoop = async (messages: Message[]): Promise<Message[]> => {
            if (!chatServiceRef.current || !textToSpeechServiceRef.current || !abortControllerRef.current) {
                return messages;
            }
            ...
            while (tries < 4 && isRespondingRef.current) {
                ...
                let content = "";
                messages.push({ role: "assistant", content, stats: { timestamp: new Date().toISOString() } });
                for await (const chunk of stream) {
                    ...
                    content += delta;
                    messages = updateLastMessage(messages, {role: "assistant", content});
        ...
        const userMessage: Message = { role: "user", content: message, stats: { timestamp: new Date().toISOString() } };
        if (message === "") {
        ...
        setMessages(currentMessages => {
            ...
            return [
                ...newMessages,
                { role: "assistant", content: "", stats: { timestamp: new Date().toISOString() } }
            ];
        });
