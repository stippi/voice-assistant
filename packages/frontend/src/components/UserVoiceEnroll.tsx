...
export function UserVoiceEnroll({ user, setUserVoiceProfileId, onClose }: Props) {
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        {isLoaded && !enrolling && (
          <DialogContentText>Press Start and keep speaking until the circle closes.</DialogContentText>
        )}
        {!isLoaded && <DialogContentText>Loading...</DialogContentText>}
        {enrolling && <DialogContentText>{feedback}</DialogContentText>}
        {isLoaded && (
          <Box
            sx={{
              top: 24,
      ...
      <DialogActions>
        ...
        <Button disabled={enrolling} onClick={startEnrollment}>
          Start
        </Button>
      </DialogActions>
    </Dialog>
